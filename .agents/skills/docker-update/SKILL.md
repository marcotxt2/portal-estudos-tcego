...
Esta skill automatiza o processo de reconstrução, atualização e verificação de saúde dos containers Docker do projeto TurnoLivre (`backend`, `frontend` e `db`) após modificações no código-fonte ou nas configurações de ambiente.
...
1. **Verificação Prévia de Ambiente (`.env`)**:
   - Antes de rodar o Docker Compose, verifique se os arquivos `.env` na raiz do projeto e `api/.env.dev` na pasta `api/` existem no sistema de arquivos.
   - Se algum deles não existir, interrompa ou avise o usuário orientando a cópia dos arquivos `.example` (`.env.example` -> `.env` e `api/.env.dev.example` -> `api/.env.dev`).
...
2. **Identificação do Alvo e Escopo**:
   - Determine qual serviço deve ser atualizado de acordo com o parâmetro passado ou contexto da conversa:
     ...
     - `backend`: Atualiza exclusivamente o container do Spring Boot (`turnolivre-backend`).
     - `frontend`: Atualiza exclusivamente o container do React/Vite/Nginx (`turnolivre-frontend`).
...
3. **Execução do Rebuild e Atualização**:
   ...
   - Acompanhe a saída do terminal para certificar-se de que os multi-stage builds foram concluídos com êxito.
...
5. **Verificação de Status e Saúde (Health Check)**:
   - Verifique se os containers estão rodando de forma estável com o comando:
     - `docker compose ps`
   - Se algum container (como `turnolivre-backend` ou `turnolivre-frontend`) apresentar status `Restarting`, `Exited` ou falha na inicialização, consulte os últimos registros do log (`docker compose logs --tail=30 <servico>`) para identificar a causa raiz (ex: porta ocupada, erro de conexão com banco, falha na JVM ou Nginx).
...
6. **Relatório Técnico Final**:
   ...
   - **Regra Estrita de Estilo**: Nunca utilize emojis no relatório, nos títulos ou nas tabelas, cumprindo integralmente a diretriz do `.antigravityrules`.
